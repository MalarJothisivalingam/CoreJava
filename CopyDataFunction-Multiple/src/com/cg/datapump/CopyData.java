...
public class CopyData {
	...
	public static void main(String[] args) {
		...
		Table[] tb = copyData.dataPump.getTables();
		for (int num = 0; num < tb.length; num++) {
			System.out.println("Fetching Details For Table:"+(num+1));
			if (dataPump.getTables()[num].getForceMatch().equals("true")) {
				copyData.populateMetaData(num);
			}
			Column[] colCount=copyData.dataPump.getTables()[num].getColumns();
			if(colCount.length<1)
			{
				copyData.overrideColumnMatch(false,num);
			}
			else
			{
			copyData.overrideColumnMatch(true,num);
			}
			copyData.constuctQueries(num);
			System.out.println("   Done.");
	...
	private void populateMetaData(int num) {
		fetchMetaData(sourceCon, dataPump.getTables()[num].getSourceSchema(),
				dataPump.getTables()[num].getSourceTable(), false);

		fetchMetaData(destCon, dataPump.getTables()[num].getDestSchema(),
				dataPump.getTables()[num].getDestTable(), true);
	}

	private void fetchMetaData(java.sql.Connection con, String schema,
			String table, boolean flag) {
		try {
			DatabaseMetaData md = con.getMetaData();
			ResultSet rs = md.getColumns(null, schema.toUpperCase(),
					table.toUpperCase(), "%");
			while (rs.next()) {
	...
	private void populateMappingInfo(MappingInfo mappingInfo) {
	 ...
	 private void overrideColumnMatch(boolean usecolumn,int num)
	  {
	      List list = new ArrayList();
	      if(usecolumn && dataPump.getTables()[num].getForceMatch().equals("true") && dataPump.getTables()[num].getColumns().length > 0)
	      {
	          ...
	          for(Iterator iterator = srcColumLst.iterator(); iterator.hasNext();)
	          {
	              ...
	              Column acolumn1[];
	              int l = (acolumn1 = dataPump.getTables()[num].getColumns()).length;
	      ...
	      {
	          Column acolumn[];
	          int j = (acolumn = dataPump.getTables()[num].getColumns()).length;
	          for(int i = 0; i < j; i++)
	          {
	...
	private void constuctQueries(int num) {
		StringBuffer srcBuffer = new StringBuffer("Select ");
		StringBuffer dstBuffer = new StringBuffer("Insert into "
				+ dataPump.getTables()[num].getDestSchema() + "."
				+ dataPump.getTables()[num].getDestTable() + " ( ");
		...
		srcBuffer.append(" from ")
				.append(dataPump.getTables()[num].getSourceSchema())
				.append(".").append(dataPump.getTables()[num].getSourceTable())
				.append(" ");
		if (dataPump.getTables()[num].getClause() != null) {
			srcBuffer.append(dataPump.getTables()[num].getClause());
		}
